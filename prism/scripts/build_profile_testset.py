...
# ── Configuration (must match run_evaluation_full.py exactly) ─────────────────
LAYER_NAMES    = ['layer2', 'layer3', 'layer4']
LAYER_WEIGHTS  = {'layer2': 0.15, 'layer3': 0.30, 'layer4': 0.55}
DIM_WEIGHTS    = [0.5, 0.5]    # equal H0/H1 — best empirical result
N_SUBSAMPLE    = 200
MAX_DIM        = 1

# Test-set index ranges (out of 10,000 test images)
PROFILE_IDX    = (0,    5000)   # 5000 images for profiling
CAL_IDX        = (5000, 7000)   # 2000 images for conformal calibration
VAL_IDX        = (7000, 8000)   # 1000 images for FPR verification
EVAL_IDX       = (8000, 10000)  # 2000 images held out for final evaluation
# ──────────────────────────────────────────────────────────────────────────────
...
def build_profile_testset(
    ...
    for i in tqdm(range(PROFILE_IDX[0], PROFILE_IDX[1])):
        img, _ = dataset[i]
    ...
    def compute_scores(idx_range, label):
        ...
        for i in tqdm(range(idx_range[0], idx_range[1]),
            ...
            # Re-use pre-computed diagrams if in profile range, else recompute
            if idx_range[0] == PROFILE_IDX[0]:
                local_i = i - PROFILE_IDX[0]
                dgms = {layer: all_diagrams[layer][local_i] for layer in LAYER_NAMES}
            else:
                img, _ = dataset[i]
                x = img.unsqueeze(0).to(device)
                acts = extractor.extract(x)
