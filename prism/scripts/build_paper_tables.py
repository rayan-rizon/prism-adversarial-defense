...
Emitted tables (into --out-dir, default: paper/tables/):
  ...
  adaptive_pgd.tex            -- ensemble-complete adaptive-PGD scan/confirmation
...
def table_adaptive_pgd(results_dir):
    ...
    summary_path = os.path.join(
        exp_root, 'evaluation', 'ensemble_complete_adaptive_pgd_summary.json'
    )
    if os.path.exists(summary_path):
        ...
        for lam in sorted(scan, key=lambda x: float(x)):
            row = scan[lam]
            lo, hi = row['TPR_CI_95']
            lines.append(
                f"Scan & {float(lam):.1f} & {int(row['n_adv'])} & "
                f"{_fmt(row['TPR'])} & {_fmt_ci(lo, hi)} & "
                f"{_fmt(row['TPR_on_successful_attacks'])} & {_fmt(row['FPR'])} \\\\"
            )
        if confirm:
            lo, hi = confirm['TPR_CI_95']
            lines += [
                "\\midrule",
                (
                    f"Confirm & 10.0 & {int(confirm['n_adv'])} & "
                    f"\\textbf{{{_fmt(confirm['TPR'])}}} & {_fmt_ci(lo, hi)} & "
                    f"\\textbf{{{_fmt(confirm['TPR_on_successful_attacks'])}}} & "
                    f"{_fmt(confirm['FPR'])} \\\\"
                ),
            ]
        lines += ["\\bottomrule", "\\end{tabular}"]
...
def table_campaign(results_dir):
    ...
    per_dataset_scen = defaultdict(lambda: {'ttd': [], 'fpr': [], 'l0_active_frac': []})
    for p in paths:
        d = _load_json(p)
        dataset = _dataset_label(p, d)
        for scen, v in d.items():
            ...
            if 'l0_on' in v and 'l0_off' in v:
                ttd = v['l0_on'].get('time_to_detect_queries')
                if ttd is not None:
                    per_dataset_scen[(dataset, scen)]['ttd'].append(ttd)
                fpr = v['l0_on'].get('FPR_clean_steps')
                if fpr is not None:
                    per_dataset_scen[(dataset, scen)]['fpr'].append(fpr)
                l0_frac = v['l0_on'].get('l0_active_fraction')
                if l0_frac is not None:
                    per_dataset_scen[(dataset, scen)]['l0_active_frac'].append(l0_frac)
    ...
    lines = [
        ...
        "\\begin{tabular}{llcccc}",
        "\\toprule",
        "Dataset & Scenario & Mean time-to-detect & Range & L0 active & FPR clean \\\\",
        "\\midrule",
    ]
    for dataset, scen in sorted(per_dataset_scen):
        entry = per_dataset_scen[(dataset, scen)]
        ttd = entry['ttd']
        l0 = entry['l0_active_frac']
        fpr = entry['fpr']
        if ttd:
            ttd_cell = f"${np.mean(ttd):.1f} \\pm {_sample_std(ttd):.1f}$"
            rng_cell = f"[{min(ttd)}, {max(ttd)}]"
        else:
            ttd_cell = "---"
            rng_cell = "---"
        l0_cell = f"${np.mean(l0):.3f} \\pm {_sample_std(l0):.3f}$" if l0 else "---"
        fpr_cell = f"${np.mean(fpr):.3f} \\pm {_sample_std(fpr):.3f}$" if fpr else "---"
        scen_tex = scen.replace('_', '\\_')
        lines.append(
            f"{dataset} & {scen_tex} & {ttd_cell} & {rng_cell} & {l0_cell} & {fpr_cell} \\\\"
        )
    lines += ["\\bottomrule", "\\end{tabular}"]
    return '\n'.join(lines) + '\n'
...
def table_recovery(results_dir):
    """Generate the active recovery table from recovery_uniform only."""
    exp_root = _experiments_root(results_dir)
    paths = sorted(glob(os.path.join(
        exp_root, 'recovery_uniform', 'results*recovery*seed*.json')))
    per_dataset_strat = defaultdict(list)
    per_dataset_seed = defaultdict(dict)
    for p in paths:
        d = _load_json(p)
        dataset = _dataset_label(p, d)
        meta = d.get('_meta') or {}
        seed = meta.get('seed')
        if seed is None:
            match = re.search(r'seed(\d+)', p)
            seed = match.group(1) if match else p
        for k, v in d.items():
            if k.startswith('_') or not isinstance(v, dict):
                continue
            if 'recovery_accuracy' in v:
                per_dataset_strat[(dataset, k)].append(v)
                per_dataset_seed[(dataset, seed)][k] = v['recovery_accuracy']
    if not per_dataset_strat:
        return f"% No recovery results found under {exp_root}.\n"
    ...
    lines = [
        ...
        "\\begin{tabular}{llcccc}",
        "\\toprule",
        "Dataset & Strategy & Recovery Acc. & 95\\% CI & Gap vs passthrough (pp) & Availability \\\\",
        "\\midrule",
    ]
    datasets = sorted({dataset for dataset, _ in per_dataset_strat})
    strat_order = ['reject', 'passthrough', 'tamsh_uniform', 'tamsh', 'tamsh_force']
    for dataset in datasets:
        seed_rows = [
            row for (ds, _seed), row in per_dataset_seed.items()
            if ds == dataset
        ]
        for strat in strat_order:
            if (dataset, strat) not in per_dataset_strat:
            ...
            avail = np.mean([e.get('availability', 0) for e in entries])
            label = {
                'tamsh': 'tamsh topology gate',
                'tamsh_force': 'tamsh\\_force (PGD specialist)',
                'tamsh_uniform': 'tamsh\\_uniform',
            }.get(strat, strat.replace('_', '\\_'))
            if strat in {'reject', 'passthrough'}:
                gap = '---'
            else:
                gaps = [
                    (row[strat] - row['passthrough']) * 100
                    for row in seed_rows
                    if strat in row and 'passthrough' in row
                ]
                gap = f"${np.mean(gaps):+.2f} \\pm {_sample_std(gaps):.2f}$" if gaps else '---'
            lines.append(
                f"{dataset} & {label} & {_fmt(acc)} & {_fmt_ci(lo, hi)} & {gap} & {_fmt(avail)} \\\\"
            )
    lines += ["\\bottomrule", "\\end{tabular}"]
    return '\n'.join(lines) + '\n'
