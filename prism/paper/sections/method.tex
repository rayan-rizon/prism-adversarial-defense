...
$\ell_\mathrm{ens}(x) = \mathbf{w}^\top \mathbf{f}(x) + b$, centred on
the calibration distribution. The final \prism score
$S(x) = \ell_\mathrm{ens}(x) + \beta\, S_\mathrm{TDA}(x)$
($\beta = 1$ in the deployed configuration) is the quantity fed to the
\cadg conformal calibrator. The \emph{ensemble-no-TDA} ablation zeroes
$\mathbf{f}_\mathrm{TDA}$ (and the TDA fusion term) to isolate the
marginal value of persistence features, and the \emph{channel-mask}
