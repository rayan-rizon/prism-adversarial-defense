...
topological channel: on CIFAR-10/ResNet-18 it is $+13.1$~pp mean TPR
(Full $0.918$ vs.\ ensemble-no-TDA $0.787$, Table~\ref{tab:ablation}), so
topology is a strong \emph{contributing} channel within the calibrated
ensemble rather than a standalone detector (TDA-only scores $0.031$ mean TPR).
...
streams up to $1{,}000$ queries (longer streams need $C^*{\geq}8$ or
periodic reset; App.~\ref{app:l0_stress})---a threat model absent from
prior work. Under a surrogate-free direct-score attack that drives the
single-shot tier down, \sacd flags the probing campaign and holds the
full-system miss to $0.03$ (\S\ref{sec:direct_score}). A stronger
\emph{dual-adaptive} attacker (slow-ramp under the CUSUM knee, or a minimal
${\approx}2$-query perturbation) defeats the \emph{per-image-reset} monitor
(miss $0.27$--$0.47$), but \emph{session-level} monitoring---in which the
change detector persists across the attacker's query stream---restores the
miss to ${\le}0.03$ (\S\ref{sec:campaign_robust}). Campaign detection is thus
load-bearing for adaptive robustness at session granularity, not an auxiliary
feature.
...
CIFAR-10/ViT-B/16 transfer check for FGSM, PGD-50, and Square only, and a
$224\times224$ ImageNet-100/ResNet-50 resolution-scaling check (FGSM,
PGD-50, Square; 3 seeds) that holds $0.924/1.000/0.852$ TPR at FPR
$0.065$ and confirms the conformal certificate is scale-invariant,
directly answering the ``CIFAR-only toy benchmark'' objection. On the
