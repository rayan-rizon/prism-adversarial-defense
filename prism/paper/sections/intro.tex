...
rejection. The learned router (combined $H_0{+}H_1$ Wasserstein gating)
delivers a $+10.6$~pp recovery gap over passthrough on PGD-rejected
inputs; a uniform $1/K$ control achieves exactly $0$~pp, isolating the
...
$+26.0$~pp; we report this as an upper-bound analysis against the
per-input oracle ceiling of $+36.6$~pp and discuss the residual gap to
a fully learned router in \S\ref{sec:recovery} as a target for future
work.
