...
(\S\ref{sec:direct_score}). A stronger \emph{dual-adaptive} attacker
(slow-ramp under the CUSUM knee, or a minimal ${\approx}2$-query perturbation)
defeats the \emph{per-image-reset} monitor (miss $0.27$--$0.47$), but
\emph{session-level} monitoring restores the miss to ${\le}0.03$
(\S\ref{sec:campaign_robust}); campaign detection is thus load-bearing for
adaptive robustness at session granularity, not an auxiliary feature.
