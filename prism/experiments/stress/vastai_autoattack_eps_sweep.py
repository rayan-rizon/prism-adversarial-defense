"""
Vast.ai AutoAttack epsilon sensitivity sweep.

Same protocol as vastai_autoattack.py (n=1000 per seed, 5 seeds, EVAL split,
sub-attacks APGD-CE/APGD-DLR/FAB-T/Square with Square queries=1000) at
two larger Linf budgets: 12/255 and 16/255 to complement the canonical
8/255 result (vastai_autoattack.json). Produces a TPR vs eps robustness
curve for the appendix.

Output: experiments/stress/vastai_autoattack_eps_sweep.json
"""
...
EPS_LIST = [12.0 / 255.0, 16.0 / 255.0]   # complement 8/255 already in vastai_autoattack.json
BATCH_SIZE = 250
...
def run_aa(norm_backbone, x_clean, y_true, attacks, eps, device):
    ...
    return adversary.run_standard_evaluation(x_clean, y_true, bs=BATCH_SIZE)


def main():
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f'device: {device}  AA-eps-sweep(eps_list={EPS_LIST}, attacks={SUB_ATTACKS}, '
    ...
    results = {
        ...
        'eps_list': EPS_LIST, 'sub_attacks': SUB_ATTACKS, 'square_queries': 1000,
    ...
    for seed in SEEDS:
        ...
        # Clean scoring (once per seed; same as base AA run)
        ...
        for eps in EPS_LIST:
            results['per_seed'][str(seed)]['by_eps'][str(eps)] = {'sub_attack': {}}
            for atk in SUB_ATTACKS:
                ...
                results['per_seed'][str(seed)]['by_eps'][str(eps)]['sub_attack'][atk] = {
                    'attack_success': attack_success, 'mean_Linf': float(l_inf),
                ...
                out_path = os.path.join(HERE, 'vastai_autoattack_eps_sweep.json')
    ...
    print(f'\n  eps     | attack   | success | Linf   | TPR(L1) [CI95]              | TPR(L2)  | TPR(L3)')
    for eps in EPS_LIST:
        for atk in SUB_ATTACKS:
            succ = np.mean([results['per_seed'][str(s)]['by_eps'][str(eps)]['sub_attack'][atk]['attack_success'] for s in SEEDS])
            li = np.mean([results['per_seed'][str(s)]['by_eps'][str(eps)]['sub_attack'][atk]['mean_Linf'] for s in SEEDS])
            tpr_l1_k = sum(results['per_seed'][str(s)]['by_eps'][str(eps)]['sub_attack'][atk]['TPR_L1_count'] for s in SEEDS)
            tpr_l2_k = sum(results['per_seed'][str(s)]['by_eps'][str(eps)]['sub_attack'][atk]['TPR_L2_count'] for s in SEEDS)
            tpr_l3_k = sum(results['per_seed'][str(s)]['by_eps'][str(eps)]['sub_attack'][atk]['TPR_L3_count'] for s in SEEDS)
            ...
            results['aggregate'][f'eps_{eps}_{atk}'] = {
            ...
            print(f'  {eps:.4f} | {atk:>8s} |  {succ:.3f}  | {li:.3f} | '
