"""
Vast.ai paper-grade stronger CW: kappa in {0, 10, 20}, n=1000 per seed,
5 seeds (42, 123, 456, 789, 999), EVAL split [8000, 9999).

Uses native PyTorch CW (src/attacks/cw_torch.py) with canonical
max_iter=40, binary_search_steps=5. Reports TPR at L1/L2/L3 thresholds
with Wilson 95% CIs from pooled per-seed counts.
"""
...
from src.config import (
    ...
    BACKBONE_MEAN, BACKBONE_STD, BACKBONE_NUM_CLASSES,
...
BATCH_SIZE = 250  # CW attack batch size
...
def main():
    ...
    print(f'device: {device} (n_per_seed={N_PER_SEED}, seeds={SEEDS}, kappas={KAPPAS})')
    ...
    for seed in SEEDS:
        ...
        for kappa in KAPPAS:
            t0 = time.time()
            print(f'  CW kappa={kappa} (batched)...')
            adv_chunks = []
            for s in range(0, N_PER_SEED, BATCH_SIZE):
                e = min(s + BATCH_SIZE, N_PER_SEED)
                batch = clean_stack[s:e]
                adv_b, _stats = cw_l2_attack_torch(
                    norm_backbone, batch, device,
            ...
            print(f'    attack_success={attack_success:.3f} L2={l2:.4f} '
                  f'attack_time={dt_attack:.1f}s, scoring adv...')
            ...
            results['per_seed'][str(seed)]['cw_by_kappa'][str(kappa)] = {
                ...
                'mean_L2': float(l2), 'attack_time_s': round(dt_attack, 1),
    ...
    pooled_clean = np.concatenate(clean_pool_all_scores)
    pooled_fpr = {
        'L1': float((pooled_clean > thr['L1']).mean()),
        'L2': float((pooled_clean > thr['L2']).mean()),
        'L3': float((pooled_clean > thr['L3']).mean()),
    }
    print(f'pooled clean FPR: {pooled_fpr}')
    results['aggregate']['clean'] = {
        'mean': float(pooled_clean.mean()), 'std': float(pooled_clean.std()),
        'FPR': pooled_fpr, 'n_total': n_total,
    }

    print(f'\n  kappa | attack_success | mean_L2 | TPR(L1) [CI95]              | TPR(L2)  | TPR(L3)')
    for kappa in KAPPAS:
    ...
    out_path = os.path.join(HERE, 'vastai_stronger_cw.json')
